...
def decrypt(encoded_text, cle_secrete, test_number):
    ...
    # Pour celui-ci, on n'ajoute pas de . et , à la fin des mots, parce qu'on assume que la majorité des mots se trouvent déjà dans le premier dictionnaire
    with open("liste.de.mots.francais.frgut.txt", "r", encoding="utf-8") as f:
        for word in f:
            word = word.strip()
            fr_dict.add(word)
    ...
    while (
        ...
        if len(bytes_connus) >= desired_minimum_known_bytes:
            ...
            if current_min_freq == min_min_freq and current_ratio_threshold_bytes == min_ratio_threshold_bytes and current_ratio_threshold_fr == min_ratio_threshold_fr:
                ...
                if exclude_rare_chars:
                    print("On a exclu les caractères rares, mais on n'a pas trouvé assez de substitutions. On recommence sans exclure les caractères rares.")
                    exclude_rare_chars = False
                    continue
...
def verify_substitution_in_text(byte, char, key, text):
    ...
    Si on trouve une meilleure substitution, on peut overwrite la substitution actuelle.
    '''
    known_chars = set()
